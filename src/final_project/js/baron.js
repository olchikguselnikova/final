...
module.exports.event = function event(elem, _eventNames, handler, mode) {
    let eventNames = _eventNames.split(' ')
    let prefix = mode == 'on' ? 'add' : 'remove'
...
module.exports = function log(level, msg, more) {
    let func = console[level] || console.log
    let args = [
        'Baron: ' + msg,
        more
    ]
...
let scopedWindow = g && g.window || g

let event = __webpack_require__(0).event
let css = __webpack_require__(0).css
let add = __webpack_require__(0).add
let has = __webpack_require__(0).has
let rm = __webpack_require__(0).rm
let clone = __webpack_require__(0).clone
let qs = __webpack_require__(0).qs

let _baron = baron // Stored baron value for noConflict usage
// var Item = {}
let pos = ['left', 'top', 'right', 'bottom', 'width', 'height']
...
// Some ugly vars
let opera12maxScrollbarSize = 17
// I hate you https://github.com/Diokuz/baron/issues/110
let macmsxffScrollbarSize = 15
let macosxffRe = /[\s\S]*Macintosh[\s\S]*\) Gecko[\s\S]*/
let isMacFF = macosxffRe.test(scopedWindow.navigator && scopedWindow.navigator.userAgent)

let log, liveBarons, shownErrors
...
function baron(user) {
    let withParams = !!user
    let tryNode = (user && user[0]) || user
    let isNode = typeof user == 'string' || tryNode instanceof HTMLElement
    let params = isNode ? { root: user } : clone(user)
    let jQueryMode
    let rootNode
    let defaultParams = {
        direction: 'v',
    ...
    let attr = manageAttr(rootNode, params.direction)
    let id = +attr // Could be NaN
...
function manageAttr(node, direction, mode, id) {
    let attrName = 'data-baron-' + direction + '-id'
...
function init(params) {
    let out = new baron.prototype.constructor(params)
...
baron.prototype = {
    ...
    constructor: function(params) {
        ...
        function setBarSize(_size) {
            let barMinSize = this.barMinSize || 20
            let size = _size
        ...
        function posBar(_pos) {
            if (this.bar) {
                let was = css(this.bar, this.origin.pos),
                    will = +_pos + 'px'
        ...
        this.pos = function(x) { // Absolute scroller position in px
            let ie = 'page' + this.origin.x + 'Offset',
        ...
        this.barOn = function(dispose) {
            if (this.barOnCls) {
                let noScroll = this.scroller[this.origin.client] >= this.scroller[this.origin.scrollSize]
        ...
        this.drag = function(e) {
            let rel = posToRel.call(this, this.cursor(e) - scrollerPos0)
            let sub = (this.scroller[this.origin.scrollSize] - this.scroller[this.origin.client])
        ...
        this.resize = function() {
            let self = this
            let minPeriod = (self.resizeDebounce === undefined) ? 300 : self.resizeDebounce
            let delay = 0
            ...
            function upd() {
                let offset = self.scroller[self.origin.crossOffset]
                let client = self.scroller[self.origin.crossClient]
                let padding = 0
                let was, will
                ...
                if (offset) { // if there is no size, css should not be set
                    ...
                    if (self.impact == 'scroller') { // scroller
                        let delta = offset - client + padding
        ...
        this._dumbCss = function(on) {
            ...
            let overflow = on ? 'hidden' : null
            let msOverflowStyle = on ? 'none' : null
            ...
            let scroll = on ? 'scroll' : null
            let axis = this.direction == 'v' ? 'y' : 'x'
            let scrollerCss = {}
        ...
        if (isMacFF) {
            let padding = 'paddingRight'
            let styles = {}
            // getComputedStyle is ie9+, but we here only in f ff
            let paddingWas = scopedWindow.getComputedStyle(this.scroller)[[padding]]
            ...
            let numWas = parseInt(paddingWas, 10)
...
function autoUpdateOne(MutationObserver) {
    ...
    let debouncedUpdater = self._debounce(function() {
        self.update()
    }, 300)
...
module.exports = function autoUpdateCreator(win) {
    let MutationObserver = win.MutationObserver || win.WebKitMutationObserver || win.MozMutationObserver || null
...
let _baron = __webpack_require__(3)
...
module.exports = function controls(params) {
    let forward, backward, track, screen,
        self = this,
        event
    ...
    if (params.forward) {
        ...
        event = {
            ...
            handler: function() {
                let y = self.pos() + (params.delta || 30)
        ...
        if (track) {
            event = {
                ...
                handler: function(e) {
                    ...
                    let y = self.pos() + sign * screen * self.scroller[self.origin.client]
...
module.exports = function fix(userParams) {
    ...
    function bubbleWheel(e) {
        try {
            let i = document.createEvent('WheelEvent') // i - for extra byte
    ...
    function init(_params) {
        ...
        let event = {
            ...
            handler: function() {
                ...
                let locPos = top - topFixHeights[num]
        ...
        if (params.clickable) {
            ...
            for (let j = 0; j < event.element.length; j++) {
    ...
    let fixFlag = [], // 1 - past, 2 - future, 3 - current (not fixed)
        gradFlag = []

    this.on('init scroll', function() {
        let fixState, hTop, gradState
        let i
